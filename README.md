...
I made this to learn how RAG works instead of just watching another tutorial on it. Frontend is just Streamlit, didn't want to spend time on that part.
...
- ~~Chat page crashed on load~~ — there was an indentation bug in `pages/2_Chat.py` where the response-generation block sat outside the `if question:` check, so it referenced `prompt` before it was ever defined and threw a `NameError` the moment you opened the page. Fixed by nesting that block back under `if question:`.
